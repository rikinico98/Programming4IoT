...
class DailyMonitor():
    ...
    def GET(self, *uri):
        ...
        roomID=uri[1]
        self.roomID=roomID
        r_TS = requests.get(f'{URL}/catalog/{self.roomID}/TS_utilities') # richiesta al catalog delle specifiche di ThingSpeak
        self.roomID=roomID
    ...
    def data_retrieve(self):
        ...
        d1_devName=[]
        field_tot=[]
        dev = requests.get(f'{URL}/catalog/{self.roomID}/devices') # richiesta dei device per la stanza
        if dev.status_code==200:
            ...
            for n in range(len(d_ID)):
                chiavi=d_ID[n]
                print(chiavi[0])
                field = requests.get(f'{URL}/catalog/{self.roomID}/{chiavi[0]}/get_field') # richiesta del field thingspeak per ogni device
            ...
            for i in range(len(measure_type)):
                ...
                if medie[i] != None:
                    ...
                    message =message+(f'\nSTATISTICS: DEVICE-->{str(d_devName[i])}\n{measure_type[i]}\nMEAN VALUE: {medie[i] :.2f}\nMAX VALUE: {massimo[i] :.2f}\nMIN VALUE: {minimo[i] :.2f}\nLAST UPDATE: {last_up[i]}\nGeneral trend available at:{ThingSpeak_link}')
...
if __name__ == '__main__':
    ...
    cherrypy.tree.mount(DailyMonitor(),'/',conf)
    cherrypy.config.update({'server.socket_host': '0.0.0.0','server.socket_port':8030})
