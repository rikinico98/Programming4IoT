...
class WareBot:
    ...
    def __init__(self,APIs):
        #############--IP per le richieste--#################
        self.catalog_URL = APIs['catalogURL']
        r = requests.get(f'{self.catalog_URL }/catalog/Telegram')
        body = json.dumps(r.json(), indent=4)
        TelegramDict = json.loads(body)
        conf = TelegramDict['Telegram']
        port = conf['brokerPort']
        self.database_stats_URL = APIs['databaseStatsURL']
        self.stored_products_IP = APIs['soldProductsURL']
        # Definisco il nome del client, topic, e MyMqtt object per la
        # registrazione MQTT
        self.clientID = conf['clientID_MQTT']
        self.topic = conf['mqttTopic']
        self.broker = conf['brokerURL']
        self.client = MyMQTT(self.clientID, self.broker, port, self)
        # Local token
        self.tokenBot = conf['telegramToken']
...
##########################################################################
    ...
    def on_chat_message(self, msg):
        ...
        # CONTROLLO SE L'USERID ASSOCIATO AL CHAT_ID HA SUBITO QUALCHE VARIAZIONE
                        ...
                        for status in self.sessionStatus:
                            if status['chatID'] == chat_ID:
                                if status['isRegistered']:
                                    ...
                                    requests.put(
                                        url=f'{self.catalog_URL}/catalog/{message}/change_chatID', data=body)
                        ...
                        if productReq['chatID'] == chat_ID:
                            ...
                            if 'quantity' in productReq.keys():
                                ...
                                if action == "delete":
                                    ...
                                    r = requests.get(
                                        f"{self.database_stats_URL}/db/stored/{roomID}/{barcode}/all")
                                    body = json.dumps(r.json(), indent=4)
                                    ...
                                    if len(productsList) > 0:
                                        ...
                                        if quantityRequested > quantityDB:
                                            self.productRequestStatus.remove(
                                                productReq)
                                ...
                                if action == 'delete':
                                    word = 'sold'
                                    r = requests.delete(
                                        url=f"{self.stored_products_IP}/db/{roomID}/{barcode}/{action}", data=body)
                                    self.productRequestStatus.remove(
                                        productReq)
                                else:
        ...
        if query_data == 'accedi':
            ...
            for status in self.sessionStatus:
                if status['chatID'] == chat_ID:
                    userID = status['userID']
                    break
            r = requests.get(f'{self.catalog_URL}/catalog/rooms')
            body = json.dumps(r.json(), indent=4)
            roomDict = json.loads(body)
            r = requests.get(
                f'{self.catalog_URL}/catalog/{userID}/assigned_rooms')
            body = json.dumps(r.json(), indent=4)
            ...
            for room in rooms:
                if room['roomID'] in assignedRooms:
                    r = requests.get(
                        f"{self.catalog_URL}/catalog/{room['roomID']}/users")
                    body = json.dumps(r.json(), indent=4)
                    userDict = json.loads(body)
                    users = userDict['user']
                    userToOutput = []
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db/statistics/mean/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            mean = d_data["mean"]
            # Check if the result is correct:
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db/statistics/total/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            total = d_data["total"]
            # Check if the result is correct:
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db/sold/most/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            products = d_data["products"]
            # Check if the result is correct:
            ...
            for productRequest in self.productRequestStatus:
                if productRequest['action'] == 'most':
                    if productRequest['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/{roomID}/most/all')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
            ...
            for request in self.dateRequestedMost:
                if request['requested'] == 'month':
                    if request['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/most/month/all?month={month}')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
                        ...
                        for request in self.dateRequestedMost:
                            if request['requested'] == 'month':
                                if request['chatID'] == chat_ID:
                                    ...
                                    # Request to Statistics_products server
                                    r_data = requests.get(
                                        f'{self.database_stats_URL}/db/sold/{roomID}/most/month/all?month={month}')
                                    j_data = json.dumps(
            ...
            for request in self.dateRequestedMost:
                if request['requested'] == 'year':
                    if request['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/most/year/all?year={year}')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
                        ...
                        for request in self.dateRequestedMost:
                            if request['requested'] == 'year':
                                if request['chatID'] == chat_ID:
                                    ...
                                    # Request to Statistics_products server
                                    r_data = requests.get(
                                        f'{self.database_stats_URL}/db/sold/{roomID}/most/year/all?year={year}')
                                    j_data = json.dumps(
                                        r_data.json(), indent=4)
                                    d_data = json.loads(j_data)
                                    products = d_data["products"]
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db/sold/least/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            products = d_data["products"]
            # Check if the result is correct:
            ...
            for productRequest in self.productRequestStatus:
                if productRequest['action'] == 'least':
                    if productRequest['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/{roomID}/least/all')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
            ...
            for request in self.dateRequestedLeast:
                if request['requested'] == 'month':
                    if request['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/least/month/all?month={month}')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
                        ...
                        for request in self.dateRequestedLeast:
                            if request['requested'] == 'month':
                                if request['chatID'] == chat_ID:
                                    ...
                                    # Request to Statistics_products server
                                    r_data = requests.get(
                                        f'{self.database_stats_URL}/db/sold/{roomID}/least/month/all?month={month}')
                                    j_data = json.dumps(
                                        r_data.json(), indent=4)
                                    d_data = json.loads(j_data)
                                    products = d_data["products"]
            ...
            for request in self.dateRequestedLeast:
                if request['requested'] == 'year':
                    if request['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/sold/least/year/all?year={year}')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
                        ...
                        for request in self.dateRequestedLeast:
                            if request['requested'] == 'year':
                                if request['chatID'] == chat_ID:
                                    ...
                                    # Request to Statistics_products server
                                    r_data = requests.get(
                                        f'{self.database_stats_URL}/db/sold/{roomID}/least/year/all?year={year}')
                                    j_data = json.dumps(
                                        r_data.json(), indent=4)
                                    d_data = json.loads(j_data)
                                    products = d_data["products"]
            ...
            for productRequest in self.productRequestStatus:
                if productRequest['action'] == 'stored':
                    if productRequest['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/stored/{roomID}/all')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        products = d_data["products"]
                        # Check if the result is correct:
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db//statistics/count/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            count = d_data["count"]
            # Check if the result is correct:
            ...
            for productRequest in self.productRequestStatus:
                if productRequest['action'] == 'count':
                    if productRequest['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/statistics/count/{roomID}/all')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        count = d_data["count"]
                        # Check if the result is correct:
            ...
            # Request to Statistics_products server
            r_data = requests.get(
                f'{self.database_stats_URL}/db/statistics/mode/all')
            j_data = json.dumps(r_data.json(), indent=4)
            d_data = json.loads(j_data)
            mode = d_data["mode"]
            # Check if the result is correct:
            ...
            for productRequest in self.productRequestStatus:
                if productRequest['action'] == 'mode':
                    if productRequest['chatID'] == chat_ID:
                        ...
                        # Request to Statistics_products server
                        r_data = requests.get(
                            f'{self.database_stats_URL}/db/statistics/mode/{roomID}/all')
                        j_data = json.dumps(r_data.json(), indent=4)
                        d_data = json.loads(j_data)
                        mode = d_data["mode"]
                        # Check if the result is correct:
            ...
            for request in self.threshold:
                if request['chatID'] == chat_ID:
                    ...
                    # Request to Statistics_products server
                    r_data = requests.get(
                        f'{self.database_stats_URL}/db/stored/threshold/all?th={threshold}')
                    j_data = json.dumps(r_data.json(), indent=4)
                    d_data = json.loads(j_data)
                    products = d_data["products"]
                    # Check if the result is correct:
                    ...
                    if productRequest['chatID'] == chat_ID:
                        ...
                        for request in self.threshold:
                            if request['chatID'] == chat_ID:
                                ...
                                # Request to Statistics_products server
                                r_data = requests.get(
                                    f'{self.database_stats_URL}/db/stored/{roomID}/threshold/all?th={threshold}')
                                j_data = json.dumps(r_data.json(), indent=4)
                                d_data = json.loads(j_data)
                                products = d_data["products"]
                                # Check if the result is correct:
            ...
            for status in self.sessionStatus:
                if status['chatID'] == chat_ID:
                    ...
                    break
            roomID = query_data
            dm = DailyMonitor(roomID, self.catalog_URL)
    ...
    def roomRoutine(self, chat_ID, userID, routineContext):
        ...
        body = json.dumps(r.json(), indent=4)
        roomDict = json.loads(body)
        r = requests.get(f'{self.catalog_URL}/catalog/{userID}/assigned_rooms')
        body = json.dumps(r.json(), indent=4)
        assignedDict = json.loads(body)
                ...
                if room['roomID'] in assignedRooms:
                    ...
                    for productReq in self.productRequestStatus:
                        ...
                        if productReq['action'] == 'stats':
                            ...
                            inline.append(
                                InlineKeyboardButton(
                                    text=text, callback_data='Product_scan'))
